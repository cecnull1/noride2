...
fun bnCLogic1(entity: Entity, persistentData: CompoundTag) {
    if (entity is Player) entity serverRun {
        if (persistentData nbtNotIn Constant.NBTKeys.BetterNeonMod.NORIDE2_BN_C_JH_TF_TYPE) return
        val transfurType: String = getTransfurType(
            persistentData.getDouble(Constant.NBTKeys.BetterNeonMod.NORIDE2_BN_C_JH_TF_TYPE).toInt()
        )
        entity changedModTransfur transfurType
    }
}

fun getTransfurType(number: Number, defaultTransfurType: String = Constant.TransfurVariantType.Changed.LATEX_DARK_LATEX_YUFENG) =
    when (number) {
